...
def remove_autostart():
    try:
        key = winreg.OpenKey(winreg.HKEY_CURRENT_USER, r"Software\Microsoft\Windows\CurrentVersion\Run", 0, winreg.KEY_SET_VALUE)
        winreg.DeleteValue(key, "AudioService")
        winreg.CloseKey(key)
    except Exception:
        pass

def create_tray_icon(on_exit, on_remove):
    ...
    return pystray.Icon("sndsrv", img, "Audio Service", menu)
...
class PrankApp:
    ...
    def run(self):
        try:
            keyboard.add_hotkey("ctrl+alt+shift+f12", self.remove_and_exit)
        except Exception:
            pass
        if self.config.get("tray_icon", True):
            icon = create_tray_icon(self.exit_app, self.remove_and_exit)
            icon.run()
        else:
            while self.running:
                time.sleep(1)
